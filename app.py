...
.warning-box {{ background: #fef3c7; border: 2px solid #d97706; color: #92400e; padding: 12px; border-radius: 5px; margin-top: 5px; font-size: 14px; }}
...
def purchase_new_form(request: Request, supplier_id: str = ""):
    ...
    <div class="card">
        <form method="post" action="/purchases/new" id="purchase-form">
            ...
            <div style="background:#dbeafe;padding:15px;border-radius:8px;margin:15px 0;text-align:center;">
                <div style="font-size:14px;color:#666;">Live Total</div>
                <div id="live-total" style="font-size:24px;font-weight:bold;color:#1e40af;">GHS 0.00</div>
            </div>
            ...
            <input type="number" step="0.01" name="amount_paid" id="amount_paid_field" value="0" min="0">
            <div id="overpay-warning" style="display:none;background:#fef3c7;border:2px solid #d97706;padding:12px;border-radius:5px;margin-top:5px;color:#92400e;font-size:14px;"></div>
    ...
    function getTotal() {{
        ...
        var costs = document.getElementsByName('unit_cost[]');
        var total = 0;
    ...
    function updateTotal() {{
        var total = getTotal();
        document.getElementById('live-total').innerText = 'GHS ' + total.toFixed(2);
        checkOverpayment();
    }}

    function checkOverpayment() {{
        var total = getTotal();
        var paid = parseFloat(document.getElementById('amount_paid_field').value) || 0;
        var warning = document.getElementById('overpay-warning');
        if (paid > total + 0.01) {{
            warning.style.display = 'block';
            warning.innerHTML = '⚠️ <strong>Warning:</strong> You are paying GHS ' + paid.toFixed(2) + ' but the total is only GHS ' + total.toFixed(2) + '. Overpayment: GHS ' + (paid - total).toFixed(2);
        }} else {{
    ...
    document.addEventListener('input', function(e) {{
        ...
        if (e.target.id === 'amount_paid_field') {{
            checkOverpayment();
        }}
    }});
    ...
    document.getElementById('purchase-form').addEventListener('submit', function(e) {{
        ...
        var paid = parseFloat(document.getElementById('amount_paid_field').value) || 0;
        if (paid > total + 0.01) {{
            if (!confirm('⚠️ You are paying GHS ' + paid.toFixed(2) + ' but the total is GHS ' + total.toFixed(2) + '.\\n\\nOverpayment: GHS ' + (paid - total).toFixed(2) + '.\\n\\nContinue anyway?')) {{
                e.preventDefault();
                return false;
            }}
        }}
    ...
    window.addEventListener('load', function() {{ updateTotal(); }});
    </script>
    """
...
async def purchase_new(request: Request):
    ...
    if amount_paid > total + 0.01:
        log(username, "overpayment_warning", f"Purchase overpayment: paid GHS {amount_paid:,.2f} but total was only GHS {total:,.2f}", "warning")
...
async def expense_new(request: Request, expense_date: str = Form(...), category_id: str = Form(...), amount: float = Form(...), description: str = Form(""), paid_to: str = Form(""), payment_method: str = Form("Cash")):
    ...
    log(username, "expense_add", f"Added expense: '{cat_name}' GHS {amount:,.2f}", "info")
...
def alerts_page(request: Request):
    ...
    <div class="card alert-box">
        ...
        <p style="font-size:18px;margin:10px 0 0 0;">
            <strong>{len(out_of_stock)}</strong> items OUT OF STOCK · <strong>{len(low_stock)}</strong> items running LOW
        </p>
    </div>
    <div class="card">
...
def cart_page(request: Request):
    ...
    <div class="card">
        ...
        <form method="post" action="/cart/checkout" id="checkout-form">
            ...
            <input type="number" step="0.01" name="amount_paid_now" id="amount_paid_now" value="0" min="0">
            <div id="sale-overpay-warning" style="display:none;background:#fef3c7;border:2px solid #d97706;padding:12px;border-radius:5px;margin-top:5px;color:#92400e;font-size:14px;"></div>
            <br>
    ...
    <script>
    function checkSaleOverpay() {{
        var total = {total};
        var paid = parseFloat(document.getElementById('amount_paid_now').value) || 0;
        var warning = document.getElementById('sale-overpay-warning');
        if (paid > total + 0.01) {{
            warning.style.display = 'block';
            warning.innerHTML = '⚠️ <strong>Warning:</strong> Customer is paying GHS ' + paid.toFixed(2) + ' but the total is GHS ' + total.toFixed(2) + '. Overpayment: GHS ' + (paid - total).toFixed(2);
        }} else {{
            warning.style.display = 'none';
        }}
    }}
    document.getElementById('amount_paid_now').addEventListener('input', checkSaleOverpay);
    document.getElementById('checkout-form').addEventListener('submit', function(e) {{
        var total = {total};
        var paid = parseFloat(document.getElementById('amount_paid_now').value) || 0;
        if (paid > total + 0.01) {{
            if (!confirm('⚠️ Customer is paying GHS ' + paid.toFixed(2) + ' but total is GHS ' + total.toFixed(2) + '. Continue?')) {{
                e.preventDefault();
                return false;
            }}
        }}
...
async def cart_checkout(request: Request, customer_id: str = Form(""), discount_type: str = Form("none"), discount_value: float = Form(0), payment_method: str = Form("Cash"), amount_paid_now: float = Form(0)):
    ...
    if amount_paid_now > final_total + 0.01:
        log(username, "overpayment_warning", f"Sale {invoice_no}: customer paid GHS {amount_paid_now:,.2f} but total was GHS {final_total:,.2f}", "warning")
    ...
    log(username, "sale", f"Sale {invoice_no} - Customer: {customer_name or 'Walk-in'} - Total: GHS {final_total:,.2f}", "info")
...
def receipt(request: Request, sale_id: int):
    ...
    <div class="card no-print" style="text-align:center;background:#dcfce7;border:2px solid #16a34a;">
        ...
        {f'<p style="margin-top:10px;color:#666;font-size:13px;">Customer phone: +{customer_phone}</p>' if customer_phone else '<p style="margin-top:10px;color:#d97706;font-size:13px;">ℹ️ Customer phone not saved</p>'}
    </div>
...
def activity_page(request: Request, user_filter: str = "", type_filter: str = "", severity_filter: str = "", start: str = "", end: str = "", search: str = ""):
    ...
    overpayments = len([lg for lg in logs if lg.get("action_type") == "overpayment_warning"])
    ...
    <div class="grid">
        <div class="card stat"><div class="num" style="color:#dc2626;">{overpayments}</div><div class="label">Overpayment Warnings</div></div>
        <div class="card stat"><div class="num" style="color:#666;">—</div><div class="label">—</div></div>
    </div>
    ...
    <div class="card">
        ...
        <form method="get" action="/activity">
            <div style="display:flex;gap:10px;flex-wrap:wrap;">
                <div style="flex:1;min-width:150px;"><label>User</label><select name="user_filter"><option value="">All Users</option>{user_options}</select></div>
                <div style="flex:1;min-width:150px;"><label>Action Type</label><select name="type_filter"><option value="">All Types</option>{type_options}</select></div>
                <div style="flex:1;min-width:150px;"><label>Severity</label><select name="severity_filter"><option value="">All</option><option value="info">Info</option><option value="warning">Warning</option><option value="danger">Danger</option></select></div>
            </div>
            <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:10px;">
                <div style="flex:1;min-width:150px;"><label>From</label><input type="date" name="start" value="{start}"></div>
                <div style="flex:1;min-width:150px;"><label>To</label><input type="date" name="end" value="{end}"></div>
                <div style="flex:1;min-width:200px;"><label>Search</label><input type="text" name="search" value="{search}"></div>
            </div>
...
def export_activity(request: Request):
    ...
    for lg in logs:
        ws.append([str(lg.get("created_at", ""))[:19].replace("T", " "), lg.get("username", ""), lg.get("action_type", ""), lg.get("severity", ""), lg.get("description", "")])
...
def users_add_form(request: Request):
    ...
    <div class="card">
        <form method="post" action="/users/add">
            ...
            <label>Password</label><input type="text" name="password" required minlength="4">
...
async def users_add(request: Request, username: str = Form(...), password: str = Form(...), full_name: str = Form(""), role: str = Form("cashier")):
    ...
    if not existing:
        ...
        log(current, "user_add", f"Created new user '{username}' ({role})", "warning")
...
def users_edit_form(request: Request, user_id: int):
    ...
    <div class="card">
        <form method="post" action="/users/edit/{user_id}">
            <label>Full Name</label><input type="text" name="full_name" value="{u.get('full_name','')}">
            <label>Password (leave empty to keep current)</label><input type="text" name="password" placeholder="New password">
            <label>Role</label>
            <select name="role">
            ...
            <select name="is_active">
                ...
                <option value="false" {'selected' if not u.get('is_active') else ''}>Inactive</option>
            </select>
        ...
        <form method="get" action="/margins" style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end;">
            <div style="flex:1;min-width:180px;">
                ...
                <select name="filter_type">
                    ...
                    <option value="profit">Only Profitable</option>
                    <option value="loss">Only Loss/No Profit</option>
                    <option value="lowmargin">Low Margin (below 10%)</option>
                    <option value="highmargin">High Margin (30%+)</option>
                </select>
            </div>
            <div style="flex:1;min-width:180px;">
                <label>Sort By</label>
                <select name="sort_by">
                    ...
                    <option value="profit">Total Profit (high → low)</option>
                    <option value="qty">Qty Sold (high → low)</option>
...
async def customer_add(request: Request, name: str = Form(...), phone: str = Form(""), address: str = Form(""), notes: str = Form("")):
    ...
    log(username, "customer_add", f"Added customer '{name}'", "info")
...
def customer_statement(request: Request, customer_id: int, start: str = "", end: str = ""):
    ...
    <div style="max-width:800px;margin:0 auto;">
        <div class="card" style="padding:30px;">
            ...
            <div class="table-wrap"><table style="min-width:100%;font-size:14px;">
                ...
                <tbody>
                    <tr style="background:#f3f4f6;"><td style="padding:10px;">{start}</td><td colspan="4" style="padding:10px;"><strong>Opening Balance</strong></td><td style="padding:10px;text-align:right;"><strong>GHS {opening_balance:,.2f}</strong></td></tr>
                    {tx_rows}
...
async def customer_pay(request: Request, customer_id: int, amount: float = Form(...), payment_method: str = Form("Cash"), note: str = Form("")):
    ...
    log(username, "customer_payment", f"Recorded payment of GHS {amount:,.2f} from '{c.get('name','')}'", "info")
...
def categories_list(request: Request):
    ...
    <div class="card">
        ...
        <form method="post" action="/categories/add">
            <label>Name</label><input type="text" name="name" required placeholder="e.g. Roofing">
